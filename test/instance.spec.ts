...
import { getAjaxRequest } from './heloper'
...
describe('instance', () => {
    ...
    test('should make a put method', () => {
      ...
      test('should make a patch method', () => {
            const instance = axios.create()
     ...
     test('should make a options method', () => {
            const instance = axios.create()
      ...
      test('should make a delete method', () => {
            const instance = axios.create()
      ...
      test('should make a head method', () => {
            const instance = axios.create()
    ...
    test('should have interceptor on the instance', done => {
         ...
         const iunstance = axios.create()

        instance.interceptor.request.use(config => {
           config.withCrentials = true
          return cofig
        })
        ...
        getAjaxRequest().then(request => {
           ...
           setTimeout(() => {
              expect(response.config.timeout).toEqual(0)
               expect(response.config.withCredentails).toEqual(true)
              done()
           }, 100)
        })
    })
     ...
     test('should get the computed uri', () => {
         ...
         expect(axios.getUri(fakeConfig)).toBe(
           'https://www.baidu.com/user/12345?idClient=1&isTest=2&testString=thisIsATest'
         )
    })
