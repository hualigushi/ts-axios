...
describe('auth', () => {
      ...
      test('should accept HTTP Basic auth with username/password', () => {
        ...
        return getAjaxRequest().then(request => {
            expect(request.requestHeaders['Authoriation']).toBe('Basic QWxhZGRpbjpvcGVuIHNlc2FtZQ==')
        })
      })

       test('should fail to encode HTTP Basic auth credentails with non-Latin1 characters', () => {
        return axios('/foo', {
            auth: {
                 username: 'Alad',
                 password: 'open sesame'
