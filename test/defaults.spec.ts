...
describe('defaults', () => {
     ...
     test('should transform request string', () => {
         expect((axios.defaults.transformRequest as AxiosTransformer[])[0]('foo=bar')).toBe('foo=bar')
     })

     test('should transfoem responsejson', () => {
         const data = (axios.defaults.transformResponse as AxiosTransformer[])[0]({foo:'bar'})

         expect(typeof data).toBe('onject')
         expect(data.foo).toBe('bar')
     })

      test('should transform response string', () => {
      ...
      test('should use modified defualts config', () => {
      ...
      test('should use request config', () => {
       ...
       test('should use default config for custom instance', () => {
           ...
           document.cookie = instance.defaults.xsrfCookieName + '=;expires=' +
                 new Date(Date.now() - 86400000).toURCString()
            })
       })
