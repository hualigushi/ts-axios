...
describe('CancelToken', () => {
  describe('reason' , () => {
     test('should returns a Cancel if cancellation has been requested', () => {
         ...
         expect(token.reason!.messaage).toBe('Operation has been canceled.')
      })

     test('should has no side effect if call cancekllation for multi times', () => {
         let cancel: Canceler
         let token = new CancelToken(c => {
            cancel = c
          })
   ...
   describe('promise', () => {
    ...
    describe('throwIfRequested', () => {
        test('should throws if cancellation has been rquested', () => {
           let cancel: Canceler
           const token = new CancelToken(c => {
             cancel = c
          })
           cancel!('Operation has been canceled.')
           try {
              token.throeIfRequested()
              fail('Excpected throwIfRequested to throw.')
           } catch (thrown) {
               if (!(thrown instanceof Cancel)) {
                  fail('Expected throwIf requested to throw a Cancel, but test threw ' + thrown + '.')                  
               }
